...
if( !isset($_REQUEST['id']) || !isset($_REQUEST['type']) ) {
    ...
    if( ($_REQUEST['type'] != 'top-category') && ($_REQUEST['type'] != 'mid-category') && ($_REQUEST['type'] != 'end-category') ) {
        ...
        $statement = $pdo->prepare("SELECT * FROM tbl_categoria_principal");
        $statement->execute();
        $result = $statement->fetchAll(PDO::FETCH_ASSOC);                            
        foreach ($result as $row) {
            $top[] = $row['tcat_id'];
            $top1[] = $row['tcat_nombre'];
        }

        $statement = $pdo->prepare("SELECT * FROM tbl_categoria_secundaria");
        $statement->execute();
        $result = $statement->fetchAll(PDO::FETCH_ASSOC);                            
        foreach ($result as $row) {
            $mid[] = $row['mcat_id'];
            $mid1[] = $row['mcat_nombre'];
            $mid2[] = $row['tcat_id'];
        }

        $statement = $pdo->prepare("SELECT * FROM tbl_categoria_baja");
        $statement->execute();
        $result = $statement->fetchAll(PDO::FETCH_ASSOC);                            
        foreach ($result as $row) {
            $end[] = $row['ecat_id'];
            $end1[] = $row['ecat_nombre'];
            $end2[] = $row['mcat_id'];
        }
            ...
            <div class="col-md-9">
                ...
                <div class="product product-cat">
                    ...
                    <div class="row">
                        ...
                        $prod_count = 0;
                        $statement = $pdo->prepare("SELECT * FROM tbl_producto");
                        $statement->execute();
                        $result = $statement->fetchAll(PDO::FETCH_ASSOC);
                        foreach ($result as $row) {
                        ...
                        if($prod_count==0) {
                            ...
                            for($ii=0;$ii<count($final_ecat_ids);$ii++) {
                                $statement = $pdo->prepare("SELECT * FROM tbl_producto WHERE ecat_id=? AND p_esta_activo=?");
